...
- 用cli开发,例如claude code,codex,opencode, 等工具时候,无法将图片粘贴进终端里.
- 需要手动打开中转工具，打断工作流。
- 不同平台剪贴板行为差异大，体验不一致。
...
- 文本同步：系统原生 `Ctrl+C` / `Cmd+C` 后自动广播；异机立即进入系统剪贴板，本机自发自收只记历史不二次改写剪贴板。
- 图片同步：截图、剪贴板图片、单个图片文件即时广播；异机收到后自动进入系统剪贴板，普通 `Ctrl+V` / `Cmd+V` 由系统原生处理。
...
| `Ctrl+Alt+V` / `Cmd+Option+V` | 图片路径模式 | 只处理截图、剪贴板图片、单个图片文件。把 `tmp` 里的目标路径文本放进系统剪贴板，不自动输入或粘贴。 |
| `Ctrl+Alt+Shift+V` / `Cmd+Shift+Option+V` | 真实内容模式 | 处理图片和文件。把真实图片文件或文件结果放入系统文件型剪贴板，不自动模拟粘贴。 |

再说得更直白一点：

- 文本：发送端 `Ctrl+C` 后，异机收到就会进系统剪贴板，通常直接 `Ctrl+V` 就能粘贴。
- 图片：发送端 `Ctrl+C` 后，异机收到就会进入系统剪贴板，通常直接用系统原生 `Ctrl+V` / `Cmd+V` 粘贴图片；专用热键仅用于取得 tmp 路径或图片文件。
- 自发自收：自己收到自己的广播时复用原共享历史记录，不重复新增记录，也不改写当前系统剪贴板。
...
- 异机接收端收到图片后，会加入共享历史并写入本机系统剪贴板；该程序写入事件会被监听器抑制，不会再次广播。
- 自发自收时，收到自己的图片广播会复用发送时创建的历史记录，不重复新增 Web 记录，也不二次改写系统剪贴板。
...
- `Ctrl+Alt+V`（macOS 为 `Cmd+Option+V`）：
  ...
  - 不自动输入，不自动模拟 `Ctrl+V` / `Cmd+V`；用户自己手动粘贴路径。
  - 这次路径写入只是本机消费动作，不会广播出去，也不会替换最新共享项。
  - 真实图片文件可以在后台继续从内存物化到同一路径，不要求先完全落盘再给路径。
  - 普通文件、文件夹、多文件按这组热键不会触发路径输出。
  - 如果最新共享图片来自本机，发送端本机按这组热键也会把图片物化到本机 `tmp` 目录，并把这份本机图片路径写入系统剪贴板。
- `Ctrl+Alt+Shift+V`（macOS 为 `Cmd+Shift+Option+V`）：
  - 图片会确保本机 `tmp` 目录里已有真实图片文件，然后把这份真实图片文件放进系统文件型剪贴板。
  - 图片写入完成后不自动模拟 `Ctrl+V` / `Cmd+V`；用户自己手动粘贴图片文件。
  - 文件类内容第一次触发会启动或复用传输，把真实结果准备到本机 `tmp` 目录。
  - 如果本次触发启动了传输，传输完成后会自动把 `tmp` 里的真实文件结果放进系统文件型剪贴板；多文件会保留原目录结构和文件名。
  - 文件类内容已经准备完成时，再次触发会立即复用 `tmp` 结果并写入系统文件型剪贴板。
  - 同一条共享文件内容无论重复按多少次，都只传输一次；后续只复用本机已经准备好的 `tmp` 结果。
  - 真实内容写入只是本机消费动作，不会广播出去，也不会替换最新共享项。
  - 如果最新共享图片来自本机，发送端本机按这组热键也会把本机 `tmp` 目录中的真实图片文件写入系统文件型剪贴板；文件类自发自收仍保留 tmp 准备和传输复用流程。
...
- 每个客户端只维护自己的本机结果：
  ...
  - 本机已接收的真实文件
  - 本机多文件解压目录
- 自发自收时，图片仍可通过 `Ctrl+Alt+V` / `Ctrl+Alt+Shift+V` 消费为本机 `tmp` 路径或本机真实图片文件；文件仍然走和异机接收端相同的 `tmp` 目录准备逻辑，而不是直接复用发送端当时的原始源路径。
- 广播事实只来自用户新的系统复制/截图产生的新共享内容。每次广播都应该代表不同的源内容事实，而不是同一个文件或同一张图片被本机消费动作写入系统剪贴板后又触发多次复制和广播。
...
- Linux Wayland：剪贴板链路正式支持；全局热键注册会先尝试 portal，再回退到 X11 / XWayland，属于 best-effort。
